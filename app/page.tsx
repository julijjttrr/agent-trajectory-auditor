  ...
  async function runDemo() {
    ...
    try {
      const response = await fetch('/api/audit', {
        ...
        body: JSON.stringify({ objective: 'Find the best plan for customer #1842', actions: demo.map(({ action, tool }) => ({ action, tool })) }),
      })
  ...
  return (
    <main className="min-h-screen bg-[#07090d] text-white">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
        ...
        <div className="flex items-center gap-3 text-sm text-zinc-400"><span>Auditor</span><a href="https://github.com/julijjttrr/agent-trajectory-auditor" target="_blank" rel="noreferrer" className="rounded-lg border border-white/10 px-3 py-2 hover:bg-white/5">GitHub</a></div>
      </nav>
      <section className="mx-auto max-w-7xl px-6 pb-16 pt-16">
        <div className="max-w-3xl">
          ...
          <div className="mt-8 flex gap-3"><button onClick={runDemo} disabled={running} className="rounded-xl bg-white px-5 py-3 font-medium text-black disabled:opacity-50">{running ? 'Auditing…' : 'Run demo →'}</button><button onClick={() => setShowMethodology(true)} className="rounded-xl border border-white/10 px-5 py-3 text-zinc-300">View methodology</button></div>
          {error && <p className="mt-4 text-sm text-red-400">{error}</p>}
          {result && <div className="mt-4 rounded-xl border border-emerald-400/20 bg-emerald-400/5 p-4 text-sm"><span className="font-medium text-emerald-300">Live audit complete.</span> {JSON.stringify(result)}</div>}
        </div>
        <div className="mt-16 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
          <div className="rounded-2xl border border-white/10 bg-white/[.025] p-6"><div className="mb-6 flex items-center justify-between"><div><p className="text-sm font-medium">Live trajectory</p><p className="mt-1 text-xs text-zinc-500">Objective: Find the best plan for customer #1842</p></div><span className="rounded-full border border-amber-400/20 bg-amber-400/10 px-2.5 py-1 text-xs text-amber-300">Review required</span></div><div className="space-y-2">{demo.map((item) => <div key={item.step} className="flex items-center gap-4 rounded-xl border border-white/5 bg-black/20 p-4"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-white/5 text-xs text-zinc-500">{item.step}</div><div className="min-w-0 flex-1"><p className="truncate text-sm">{item.action}</p><p className="mt-1 font-mono text-[11px] text-zinc-600">{item.tool}</p></div><span className="text-sm font-bold">{icon(item.status)}</span></div>)}</div></div>
          <div className="rounded-2xl border border-white/10 bg-white/[.025] p-6"><p className="text-sm text-zinc-400">Audit verdict</p><div className="mt-4 text-4xl font-semibold">{result?.verdict || 'READY'}</div><p className="mt-3 text-sm leading-6 text-zinc-400">{result?.summary || 'Run the demo to audit this trajectory against its objective.'}</p><div className="mt-6 space-y-3"><div className="rounded-xl border border-red-400/10 bg-red-400/5 p-4"><p className="text-sm font-medium text-red-300">Objective deviation</p><p className="mt-1 text-xs leading-5 text-zinc-500">Detect actions that are outside the declared objective or allowed scope.</p></div><div className="rounded-xl border border-amber-400/10 bg-amber-400/5 p-4"><p className="text-sm font-medium text-amber-300">Evidence required</p><p className="mt-1 text-xs leading-5 text-zinc-500">Flag risky operations so teams can review the exact evidence.</p></div></div></div>
        </div>
      </section>
      {showMethodology && <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-6" onClick={() => setShowMethodology(false)}><div className="max-w-lg rounded-2xl border border-white/10 bg-[#0c1017] p-6" onClick={e => e.stopPropagation()}><h2 className="text-xl font-semibold">How Trajectory audits an agent</h2><p className="mt-3 text-sm leading-6 text-zinc-400">We compare the declared objective with the sequence of actions and tools used by the agent, then flag deviations and risky operations.</p><button onClick={() => setShowMethodology(false)} className="mt-6 rounded-xl bg-white px-4 py-2 text-sm font-medium text-black">Close</button></div></div>}
    </main>
  )
}
